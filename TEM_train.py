...
def binary_logistic_loss(gt_scores,pred_anchors): # 目标gt_scores ,预测值 pred_anchors
    """Calculate weighted binary logistic loss
    """ 
    gt_scores = tf.reshape(gt_scores,[-1])
    pred_anchors = tf.reshape(pred_anchors,[-1])
    
    pmask = tf.cast(gt_scores>0.5,dtype=tf.float32) # {0,1}
    num_positive = tf.reduce_sum(pmask) # 西格玛 gi
    num_entries = tf.cast(tf.shape(gt_scores)[0],dtype=tf.float32)    # 总数 lw
    ratio = num_entries/num_positive # lw/sigema(gi)
