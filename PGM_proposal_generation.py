...
def iou_with_anchors(anchors_min,anchors_max,box_min,box_max):
    ...
    union_len = len_anchors - inter_len +box_max-box_min # 总的长度除去重合部分
    #print inter_len,union_len
    jaccard = numpy.divide(inter_len, union_len) # 重合部分除以总的长度
    return jaccard
...
def generateProposals(video_name,video_dict):
    ...
    start_bins[[0,-1]]=1
    for idx in range(1,tscale-1): # 计算峰值点
    ...
    for j in range(tscale):
        if start_bins[j]==1:
            ...
            xmin_score_list.append(start_scores[j]) # xmin_score_list与start有关
        if end_bins[j]==1:
            xmax_list.append(tgap/2+tgap*j)
            xmax_score_list.append(end_scores[j])  # xmax_score_list与end有关
